...
def train(x, y, x_test, y_test, learning_rate):
    ...
    test_logits = linearModel(x_test)
    y_pred = tf.round(tf.sigmoid(test_logits))
    correct_pred = tf.equal(y_pred, y_test)
    test_accuracy = tf.reduce_mean(tf.cast(correct_pred, dtype=tf.float32))
    return current_loss, test_accuracy
...
for epoch in epochs:
    current_loss, test_accuracy = train(
        ...
        learning_rate=1)
    print("Epoch %02d: loss %.8f, test %.6f" % (
        epoch, current_loss, test_accuracy
        ))
